...
export default class Dashboardnextalert extends Component {
...
const notifyid = this.props.match.params.notifyid;
...
//$('#loader_updates').fadeIn(400).html('<br><div class="loader_css" ><i class="fa fa-spinner fa-spin" style="font-size:20px"></i>  &nbsp;Please Wait, Checking if Issues has been created...</div>');
                  //var team_id = localStorage.getItem('teamidsessdata');
			$.ajax({
				url: url_forming+'alerts_notify_updates.php',
				type: 'post',
				data: {teamid_issue:teamid_issue, notifyid:notifyid, issueid_issue:issueid_issue},
				dataType: 'json',
				success: function(response){
			//alert(response);
...
openIssuesCalendar('https://qbtut.com/mondayserver/monday_db/calendar_openIssues.php?teamid='+teamingId_sess_data1);
			$('body').delegate('.calendar_calling2', 'click', function(e){
				e.preventDefault();
				openIssuesCalendar($(this).attr('href'));
			});
