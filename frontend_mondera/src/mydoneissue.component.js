...
export default class Mydoneissue extends Component {
...
var url_forming= 'https://qbtut.com/mondayserver/monday_db/';
...
const userid_issue = localStorage.getItem('useridsessdata');
//alert('i am id: ' +issueid_issue);
const userid_next = userid_issue;

const teamid_issue = localStorage.getItem('teamidsessdata');
//alert('team is: ' +teamid_issue);
const teamid_next = teamid_issue;


//alert('uid: '+userid_issue);
//alert(' team: '+teamid_issue);
...
$('#loader_check').fadeIn(400).html('<br><div class="loader_css" ><i class="fa fa-spinner fa-spin" style="font-size:20px"></i>  &nbsp;Please Wait, Checking if Issues has been created...</div>');
			...
			$.ajax({
				url: url_forming+'issues_check_mydoneissue.php',
				type: 'post',
				data: { userid_issue:userid_issue, teamid_issue:teamid_issue},
				dataType: 'json',
				success: function(response){
			
if(response == 0){
$("#result_check").html("<br /><br /><br /><div style='color:white;background:red;padding:10px;'>You Do not have Done or Completed/Closed Issues Yet</div>");   
$('#loader_check').hide();
}
$('#loader_check').hide();		           
...
//load user Profile starts here

$(document).ready(function(){

var teamid =localStorage.getItem('teamidsessdata');
$('#loader_uprofile').fadeIn(400).html('<br><div class="well" style="color:white;padding:10px;background:#3b5998"><i class="fa fa-spinner fa-spin" style="font-size:20px"></i>  &nbsp;Please Wait, Users Profile Data is being Loaded.</div>');

$.ajax({
        url: url_forming+'userprofile_call.php',
        type: 'get',
        dataType: 'JSON',
        data: {userid_issue:userid_issue, teamid_issue:teamid_issue},
        crossDomain: true,
...
//alert(respu);
if(respu){
$('#loader_uprofile').hide();
}

if(!respu){
$('#loader_uprofile').hide();

$("#result_uprofile").append("<br /><br /><div style='color:white;background:red;padding:10px;'>This User has no Profile  Yet.</div>");
$('#loader_uprofile').hide();
...
//$('#loader_members').hide();
            ...
            var rec_lengthu = respu.length;
            for(var i=0; i<rec_lengthu; i++){
              
                var id = respu[i].id;
                var userid = respu[i].id;
                var fullname = respu[i].fullname;
                var email = respu[i].email;
                var timer1 = respu[i].timer1;
                var photo = respu[i].photo;
                var token1 = respu[i].token1;
                var token2 = respu[i].token2;
                var team_id = respu[i].team_id;

            


var m_rec1u = "<div id='memberRecords' class='project_css'>" +

"<div class='row'><div class='col-sm-6'><img style='border-style: solid; border-width:3px; border-color:#ec5574; width:200px;height:200px; max-width:200px;max-height:200px;border-radius: 50%;' src='https://qbtut.com/mondayserver/uploads/" + photo +"' /></div>" +
"<div class='col-sm-6'><span style=''><b> Name: </b> " + fullname + "</span><br />" +
"<span style=''><b>Email: </b>" + email + "</span><br>" +
  "<span><b> <span style='color:#3b5998' class='fa fa-calendar'></span>Time:</b> </span>" +
"<b><span data-livestamp='" + timer1 + "'></span></b><div style='background:green;color:white;padding:10px;border-radius:20%;font-size:16px;'><i  style='font-size:20px;' class='fa fa-check'></i> User Verified</div></div></div>"+

                   "</div><p></p>";

                $("#result_uprofile").append(m_rec1u);
...
$("#nomore_content_check").html("<div style='background:purple;color:white;padding:10px;'>No More Content to Load <br> <center><button style='background:#3b5998;border:none;color:white;padding:10px;cursor:pointer' title='Refresh Page' class='reloadPageIssue'>Refresh Page</button></center> </div>");   
$('#loader_post').hide();
                }
...
$("#loader_posts").fadeIn(400).html('<br><div style="color:black;background:white;padding:10px;">Please Wait. Loading Content <i class="fa fa-spinner fa-spin" style="font-size:20px"></i></div>');
            ...
            $.ajax({
                url:url_forming+'issues_load_mydoneissue.php',
                type:'post',
                data:{queryid:queryid,page_row_call:page_row_call, teamid_issue:teamid_issue, userid_issue:userid_issue},
                dataType:'json',
                success:function(response){
...
"<div > <span data-id=\""+ issueid +"\" class='readmore-msg-btn' title='Click To Read More Issue Details' style='color:green;font-size:16px;cursor:pointer;'><b>Click to Read Issue Details...>></b></span>  <br /><span id='issueDetailsHide_"+ issueid +"'> " + details + " <span data-id=\""+ issueid +"\" style='color:red;font-size:16px;cursor:pointer' class='readmore-msg-hide-btn' title='Click To Hide Issue Details'><b>Hide Issues..<<<</b></span> </span> </div><br />" +
...
" <span class='col-sm-3'><label title='Like' id='like_"+ issueid +"'  data-teamid=\""+ team_id +"\"  data-id=\""+ issueid +"\" data-creatorname=\""+ creator_name +"\"  data-creatoruserid=\""+ creator_userid +"\"  data-title=\""+ title +"\"  style='font-size:20px;color:#3b5998;cursor:pointer;' class='like_btn fa fa-thumbs-up'> <span style='font-size:14px'>Like<span></label>"+
"(<span id='likes_"+ issueid +"'> " +total_like + "</span>)</span>"+


                        "<span class='loaderUnLike_"+ issueid +"'></span>"+
"<span class='result-unlike_"+ issueid +"'></span>"+
"<span class='col-sm-3'><label title='Un Like' id='unlike_"+ issueid +"' data-teamid=\""+ team_id +"\" data-id=\""+ issueid +"\" data-creatorname=\""+ creator_name +"\"  data-creatoruserid=\""+ creator_userid +"\"  data-title=\""+ title +"\"  style='font-size:20px;color:#3b5998;cursor:pointer;' class='unlike_btn fa fa-thumbs-down'> <span style='font-size:14px'>UnLike</span></label>"+
"(<span id='unlikes_"+ issueid +"'> " +total_unlike + "</span>)</span>"+



//"<a  className='redo' href='/dashboardnext/"+ issueid +"/"+ team_id +"/"+ creator_userid +"'>Visit 22</a><br /><br />"+

/*

" <span class='col-sm-3'><label><a title='Comments/Reply' href='issues_dashboard.html?issueid="+ issueid +"&teamid="+ team_id +"&userid="+ creator_userid +"'  style='font-size:20px;color:#3b5998;cursor:pointer;' class='fa fa-comments'> <span style='font-size:14px'>Reply on NextPage</span></a></label>"+
"(<span id='comment_"+ issueid +"'> " + total_comment + "</span>)</span>"+
*/

" <span class='col-sm-3'><label><a title='Comments/Reply' href='/dashboardnext/"+ issueid +"/"+ team_id +"/"+ creator_userid +"'  style='font-size:20px;color:#3b5998;cursor:pointer;' class='fa fa-comments'> <span style='font-size:14px'>Reply on NextPage</span></a></label>"+
"(<span id='comment_"+ issueid +"'> " + total_comment + "</span>)</span>"+
...
"<a style='margin-top:-20px;' title='View Members Profile' href='/userprofile/"+ team_id +"/"+ userid +"'  class='btn_action pull-right action_css'><span style='font-size:20px;color:black;' class='fa fa-user'></span> Users Profile </a><br>"+

//"<a  title='View Users Profile' href='profile_user.php?userid="+ userid +"&teamid="+ team_id +"&id="+ userid +"'  class='btn_action pull-right proj_css'><span style='font-size:16px;color:black;' class='fa fa-user'></span> Users Profile </a><br>"+
...
//reload page starts 
$(document).ready(function(){
$(document).on( 'click', '.reloadPageIssue', function(){
...
allIssuesCalendar('https://qbtut.com/mondayserver/monday_db/calendar_allIssues.php?teamid='+teamingId_sess_data1);
			$('body').delegate('.calendar_calling1', 'click', function(e){
				e.preventDefault();
				allIssuesCalendar($(this).attr('href'));
			});
...
// start caller1 for comments

$(document).ready(function(){
//$('.btn_action_comment').click(function(){
$(document).on( 'click', '.btn_action_comment', function(){ 
...
$('#issueid_reply').html(issueid);
$('#title_reply').html(title);
$('#status_reply').html(status);
$('#priority_reply').html(priority);

$('#issueid1').html(issueid);

// initialize local storages
localStorage.setItem('issueID101', issueid);
localStorage.setItem('title101', title);
localStorage.setItem('teamid101', teamid);




//check if reply exist for any issues yet starts here

$('#loader_check_r').fadeIn(400).html('<br><div class="loader_css" ><i class="fa fa-spinner fa-spin" style="font-size:20px"></i>  &nbsp;Please Wait, Checking if Reply Exist for any Issues Yet...</div>');
                //  var team_id = localStorage.getItem('teamidsessdata');
			$.ajax({
				url: url_forming+'issues_check_reply.php',
				type: 'post',
				data: {teamid: teamid, issueid:issueid},
				dataType: 'json',
				success: function(response_r){
			
if(response_r == 0){
$("#result_check_r").html("<div style='color:white;background:red;padding:10px;'>No Reply/Comments on This Issue Yet</div>");   
$('#loader_check_r').hide();
}
$('#loader_check_r').hide();		           
...
//check if reply exist for any issues yet ends here





// call reply load on click
loadIssueReplyMain();
...
var issueid = localStorage.getItem('issueID101');
var title   = localStorage.getItem('title101');
var teamid  = localStorage.getItem('teamid101');
...
$(".loader_comments").fadeIn(400).html('<br><div style="color:black;background:#ddd;padding:10px;"> <i class="fa fa-spinner fa-spin" style="font-size:20px"></i>&nbsp;Please Wait, Your Reply is being Published..</div>');
        $.ajax({
            url: url_forming+'comment.php',
            type: 'post',
            dataType: 'json',
            data: {comdesc:comdesc,issueid:issueid, teamid:teamid, userid_session:userid_session,title:title},          
            crossDomain: true,
	    cache:false,
            success: function(data){
...
"<a title='Click to access users Profile page'  href='profile.php?id=" + comment_userid +"' title='Click to access users page'>"+
...
// clear modal div content on modal close starts

//$('.btn_action_comment').click(function(){
$(document).on( 'click', '.closed_modal_btn', function(){ 
	
	//alert('me clicked one oo');
	 $(".clear_data_replyload").empty();
        $(".clear_data").empty();
      });

// clear modal div ends here
...
$("#nomore_content_check_reply").html("<div style='background:purple;color:white;padding:10px;'>No More Reply or comment to Load <br> <center><button style='background:#3b5998;border:none;color:white;padding:10px;cursor:pointer' title='Refresh Page' onclick='reloadPage()'>Refresh Page</button></center> </div>");   
...
$("#loader_posts_reply").fadeIn(400).html('<br><div style="color:black;background:white;padding:10px;">Please Wait. Loading Reply Comments <i class="fa fa-spinner fa-spin" style="font-size:20px"></i></div>');
            ...
            $.ajax({
                url:url_forming+'issues_load_reply.php',
                type:'post',
...
"<div class='pull-right' ><button class='action_css'>"+
"<a title='Click to access users Profile page'  href='profile.php?id=" + userid +"' title='Click to access users page'>"+
...
<br /><br />
<label style={{fontSize:'18px'}}>Your Profiles</label><br />
<div id="loader_uprofile"></div>
<div id="result_uprofile"></div>
</div><br /><br />
...
<input type="button" style={{width:'100%'}} title='LoadMore Issue Content' class="button myform_btn_css" value="LoadMore Issue Content" id="loadmore_btn" />
...
<div class="container_action">
  ...
  <div class="modal fade " id="myModal_comment" role="dialog">
    <div  class="modal-dialog modal-lg modal-appear-center1 modal_mobile_resize modaling_sizing">
      <div class="modal-content">
        <div class="modal-header" style={{color:'black', background:'#ddd'}}>
 <button type="button" class="pull-right btn btn-default closed_modal_btn" data-dismiss="modal">Close</button>

          <h4 class="modal-title">Post Reply/Comments on this Issue </h4>
        </div>
        <div class="modal-body">
...
<h4>Issue Title:  <span id="title_reply"></span></h4>
<b>Status: </b> <span id="status_reply"></span><br />
<b>Priority: </b> <span id="priority_reply"></span><br />
...
<span id='loader_check_r' class="clear_data"></span>
<span id='result_check_r' class="clear_data"></span>
 
        <div id="res_issues_reply" class="clear_data_replyload"></div>
<div id="nomore_content_check_reply"></div>
<div id="loader_check_reply" class="clear_data"></div>
...
<div id="loader_posts_reply" class="clear_data"></div><br />


<div id="commentsubmissionResult" class="clear_data"></div>
<br />
...
<br /><br />
        ...
        <div class="modal-footer modal_footer_color" style={{color:'black', background:'#ddd'}}>
          <button type="button" class="btn btn-default closed_modal_btn" data-dismiss="modal">Close</button>
        </div>
      </div>
    </div>
  </div>
